...
import { LayoutDashboard, Truck, FileText, Users, Box, Sparkles, Settings, Menu, X, Wallet } from 'lucide-react';
...
import SettingsPage from './Settings'; // <--- New Import
...
export default function App() {
  ...
  const menuItems = [
    { name: 'Dashboard', icon: <LayoutDashboard size={20} />, component: <Dashboard setActiveTab={setActiveTab} /> },
    { name: 'Fleet Manager', icon: <Truck size={20} />, component: <FleetManager /> },
    { name: 'Billing / Invoice', icon: <FileText size={20} />, component: <BillingSystem /> },
    { name: 'Labor & HR', icon: <Users size={20} />, component: <LaborManager /> },
    { name: 'Inventory', icon: <Box size={20} />, component: <InventoryManager /> },
    { name: 'Business Cashbook', icon: <Wallet size={20} />, component: <ExpenseManager /> }, 
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto relative w-full bg-slate-50">
        <header className="bg-white/80 backdrop-blur-md sticky top-0 z-30 px-8 py-4 border-b border-slate-200 flex justify-between items-center shadow-sm no-print">
          ...
          <div className="flex items-center gap-3">
            <div className="text-right hidden sm:block">
              ...
              <p className="text-xs font-black text-orange-600">Admin</p>
            </div>
        ...
        <div className="p-4 lg:p-8 max-w-7xl mx-auto pb-24">
          {/* RENDER SETTINGS IF ACTIVE, ELSE FIND TAB */}
          {activeTab === 'Settings' ? <SettingsPage /> : menuItems.find(item => item.name === activeTab)?.component}
        </div>
      </main>
    </div>
  );
